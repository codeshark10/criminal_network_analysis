...
app = FastAPI(
    ...
    description="Multi-Case API for managing graph queries, top suspects, and full entity relationships.",
    version="1.2.0",
    lifespan=lifespan
)
...
def run_ingestion_pipeline(case_id: str, file_paths: List[str]):
    ...
    print(f"\n[BACKGROUND TASK] Processing {len(file_paths)} uploaded case file(s) for case {case_id}...")
...
async def get_all_cases():
    """Fetches all case names robustly to populate workspace dropdowns."""
    query = """
    MATCH (c:Case)
...
async def create_new_case(request: CaseCreateRequest):
    """
    Creates a new empty case workspace in Neo4j so it immediately
    appears in frontend dropdowns and dashboards.
    """
    # Auto-generate a clean, unique case ID
...
import os
import shutil
from fastapi import UploadFile, File, BackgroundTasks, Query, HTTPException
...
async def upload_case_documents(
    ...
    """
    Uploads documents, auto-generates a case ID, and instantly links the
    physical files to the Case node in Neo4j for immediate dashboard visibility.
    """
    if not files:
    ...
    for file in files:
        ...
        # Prep data to pass into our Cypher query
    ...
    # --- NEW: Instantly register the Case and Documents in Neo4j ---
    query = """
    // 1. Ensure the Case node exists
    MERGE (c:Case {id: $case_id})
    ON CREATE SET 
        c.name = 'Uploaded Case ' + substring($case_id, 5),
        c.created_at = datetime(),
        c.status = 'ACTIVE'

    // 2. Iterate over all uploaded files in this batch
    UNWIND $files AS file
    MERGE (d:Entity {name: file.filename, case_id: $case_id})
    SET d:Document // Apply Document label for metrics counting
    ON CREATE SET 
        d.type = 'DOCUMENT',
    ...
    // 3. Link the document to the case
    MERGE (d)-[:BELONGS_TO]->(c)
    """

    async with driver.session(database="neo4j") as session:
        await session.run(query,  files=file_metadata_for_db)

    # --- Proceed with background tasks ---
    ...
    return UploadCasesResponse(
        ...
        message=f"Uploaded {len(saved_files)} documents and linked to case {case_id}.",
...
async def get_top_suspects(case_id: str, limit: int = Query(default=10, ge=1, le=100)):
    """Fetches top suspects specifically within the boundaries of the requested case."""
    query = """
...
async def get_full_relation_map(
    ...
    """Retrieves full case graph strictly isolated by case_id."""
    if entity_type:
        query = """
...
async def get_available_entity_types(case_id: str):
    """Returns all entity types present within a specific case."""
    query = """
...
async def list_investigators(case_id: str):
    """Lists investigators involved in the specified case."""
    query = """
    ...
    RETURN 
        i.name AS name, coalesce(i.master_role, 'INVESTIGATOR') AS master_role,
        coalesce(i.aliases, []) AS aliases, coalesce(i.mention_count, 1) AS mentions,
        connections AS degree_connections
...
async def get_investigator_cases(case_id: str, investigator_name: str):
    """Retrieves case involvement specifically filtering by case_id."""
    case_query = """
    ...
    RETURN 
        inv.name AS canonical_name, coalesce(c.name, c.id) AS case_name,
        coalesce(c.type, 'CASE') AS case_type, type(r) AS relationship,
        coalesce(r.evidence, '') AS evidence, coalesce(r.chunk_id, '') AS chunk_id
    """
    graph_query = """
    MATCH (source {case_id: $case_id})-[rel]-(target {case_id: $case_id})
    ...
    """
    async with driver.session(database="neo4j") as session:
        case_records = await (await session.run(case_query,  name=investigator_name)).data()
        graph_records = await (await session.run(graph_query,  name=investigator_name)).data()
    ...
    return InvestigatorCasesResponse(
        investigator_name=investigator_name, canonical_name=canonical_name,
        total_cases=len(cases), cases=cases, graph=graph_data
    )
...
async def get_executive_case_summary(
    ...
    """Generates an executive summary graph scoped to case_id."""
    ...
    """
    async with driver.session(database="neo4j") as session:
        records = await (await session.run(query,  max_nodes=max_nodes, min_connections=min_connections,
...
async def get_major_entities(case_id: str, limit_per_category: int = Query(default=50, ge=1, le=200)):
    """Extracts and categorizes primary entities for a specific case_id."""
    query = """
    MATCH (n {case_id: $case_id})
    ...
    async with driver.session(database="neo4j") as session:
        records = await (await session.run(query,  limit=limit_per_category)).data()
    for record in records:
...
async def get_suspect_relationships(case_id: str, suspect_name: str, limit: int = Query(default=100, ge=1, le=500)):
    """Retrieves 1-hop relationships for a suspect scoped to a specific case_id."""
    query = """
    ...
    """
    async with driver.session(database="neo4j") as session:
        records = await (await session.run(query,  suspect_name=suspect_name, limit=limit)).data()
    return parse_graph_records(records) if records else GraphResponse(nodes=[], edges=[])
...
async def get_relationship_between_persons(
    ...
    """Finds shortest paths between two individuals within a specific case."""
    query = """
    ...
    async with driver.session(database="neo4j") as session:
        records = await (
            await session.run(query,  person1=person1, person2=person2, max_depth=max_depth)).data()
    return parse_graph_records(records) if records else GraphResponse(nodes=[], edges=[])
...
async def get_dashboard_metrics(case_id: str):
    ...
    Documents, Chunks, Entities, Persons, Evidence, Relationships, Alerts, and Network Size.
    """
    query = """
    ...
    // 2. Aggregate Relationship Counts
    OPTIONAL MATCH (src {case_id: $case_id})-[r]->(tgt {case_id: $case_id})
    WITH documents, chunks, entities, persons, alerts,
         COUNT(r) AS relationships,
         SUM(CASE WHEN coalesce(r.evidence, '') <> '' THEN 1 ELSE 0 END) AS evidence
    ...
    RETURN 
        ...
        relationships, 
        alerts, 
        (entities + relationships) AS network_size
    """
    ...
    if not records:
        return DashboardMetricsResponse(
            documents=0, chunks=0, entities=0, persons=0,
            evidence=0, relationships=0, alerts=0, network_size=0
        )
    ...
    return DashboardMetricsResponse(
        ...
        persons=r.get("persons") or 0,
        evidence=r.get("evidence") or 0,
        relationships=r.get("relationships") or 0,
        alerts=r.get("alerts") or 0,
        network_size=r.get("network_size") or 0
    )
